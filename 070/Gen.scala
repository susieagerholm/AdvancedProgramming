...
case class Gen[A] (sample :State[RNG,A]) {
  ...
  def union (that :Gen[A]) :Gen[A] = 
     boolean.flatMap { x => if(x) this else that }
...
object Gen {
  ...
  def weighted[A](g1: (Gen[A],Double), g2: (Gen[A],Double)): Gen[A] = { 
    // We do not calculate a proportional share of Gen[A]. The idea is to return either 
    // g1 or g2 with a probability which corresponds to the weights
    val g1_limit =  g1._2 * (g1._2 + g2._2)  
    double.flatMap { x => if(x < g1_limit) g1._1 else g2._1 }
  }
  // 
  //
...
case class Prop (run: (TestCases,RNG) => Result) {
  ...
  def || (that :Prop) :Prop = Prop {
    (tc,rng) => this.run(tc,rng) match { 
                 case Falsified(msg, _) => that.run(tc,rng)
                 case y => y
                }
  }
