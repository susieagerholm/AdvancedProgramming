...
object data {
  ...
  trait Reduce[F[_]] {
    ...
    def toList[A] (fa: F[A]) :List[A] = fa match {
      case Empty() => Digit()
      case _ => reduceR[A, List[A]](_ +: _)(fa, Nil)
    } 
     
    // page 6
    //
    def toTree[A] (fa :F[A]) :FingerTree[A] = fa match {
      case Empty() => Empty()
      case _ => reduceR[A, FingerTree[A]]((x, y) => y.addL(x))(fa, Empty()) 
    } 
  }

  // Types for Finger trees after Hinze and Patterson (page 4)

  type Digit[A] = List[A]

  sealed trait Node[+A] {

    // uncomment the delegation once Node.toList is implemented
    //
  ...
  sealed trait FingerTree[+A] {
    ...
    // The following methods are convenience delegation so we can use
    ...
    // page 7 (but this version uses polymorphism for efficiency, so we can
  ...
  case class Empty () extends FingerTree[Nothing] {
    // page 7
    override def empty = this match {
    case NilTree() => true;
    case _ => false
    }
    
    override def nonEmpty = this match {
      case NilTree() => false;
      case _ => true
    }
  }
  ...
  // parameter names: pr - prefix, m - middle, sf - suffix
  ...
  object FingerTree extends Reduce[FingerTree] { // uncomment once the interface is implemented
    ...
    // page 5 bottom (the  gle); Actually we could use the left
    ...
    def addR[A] (fa: FingerTree[A], a: A): FingerTree[A] = fa match {
      ...
      case Deep(pr, m, Digit(e,d,c,b)) => Deep(pr, Single(Node3(e, d, c)), Digit(b, a))
    ...
    def viewL[A] (t: FingerTree[A]) :ViewL[A] = t match {
      ...
      case Deep(pr, m, sf) => ConsL(pr.head, deepL(pr.tail, m, sf)) //ConsL(pr.take(1), t)
    } 
    ...
    :FingerTree[A] = pr match {
       case Digit() => m match {
        case Empty() => Digit.toTree(sf)
        case ConsL(head, tail) => Deep(head.toList, tail, sf)
      } 
      case _ => Deep(pr, m, sf)
    } 
    ...
    :FingerTree[A] = sf match {
       case Digit() => m match {
        case Empty() => Digit.toTree(pr)
        case ConsR(head, tail) => Deep(pr, head, tail.toList)
      } 
      case _ => Deep(pr, m, sf)
    } 
    ...
    def headL[A] (t: FingerTree[A]) = t match {
      case ConsL(a, _) => a 
    }
    def tailL[A] (t: FingerTree[A]) = t match {
      case ConsL(_, b) => b 
    }
