...
object Par {
  ...
  def parFilter[A](as: List[A])(f: A => Boolean): Par[List[A]] = fork { //is call to asyncF necessary if given as arg to fork??
    val aaa: List[Par[A]] = as.filter(f).map(a => unit(a))
    sequence(aaa)
  }
  ...
  def map3[A,B,C,D] (pa :Par[A], pb: Par[B], pc: Par[C]) (f: (A,B,C) => D) = 
     map2(map2(pa, pb)((a, b) => (c: C) => f(a, b, c)), pc)(_(_))      //has problems with syntax for f in nested map2, so found solution on github 
                                                                       //its currying going on here right?? (a, b) => (c: C) => f(a, b, c). How can it be valid?
