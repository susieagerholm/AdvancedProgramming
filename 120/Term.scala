...
//
   ...
   // paper.
   def eval (term :Term) :M[Int] = term match {
     case Cons (a) => M[Int] (x => (a,x)) //RAW VALUE TO MONAD
...
//   // differs from the monadic basic one
   ...
   //The guard in the final for comprehension is the only difference between the 
   //two implementations...so the difference is small compared to the difference 
   //between the two non-monadic implementations...
}
//
...
object StateEvaluatorWithMonads {
   ...
   def eval (term :Term) :M[State] = term match {
     case Cons (a) => M.unit (a)
     case Div (t,u) =>  
       eval(t).flatMap(a => eval(u).flatMap(b => tick.map(_ => (a/b))))
...
//   // TODO: implement eval
   def eval (term :Term) :M[Int] = term match { 
     ...
     case Div (t,u) => 
       eval(t).flatMap(a => eval(u).flatMap(b => 
